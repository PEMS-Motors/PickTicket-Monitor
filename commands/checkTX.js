...
module.exports = {
    name: "starttx",
    aliases: ['tx'],
    description: "Check TX warehouse folders.",
    execute(message) {

        // Imports / Requires
        var dirwatch = require("./DirectoryWatcher.js");
        var txChannel = globalClient.channels.get(channels.locations.TX);
        ...
        // to false
        var txMonitor = new dirwatch.DirectoryWatcher("Z:\\06A-Processed", true);

        // start the monitor and have it check for updates
        // every half second.
        txMonitor.start(60000);
        
        // Log to the console when a file is removed
        txMonitor.on("fileRemoved", function (filePath) {
            txChannel.send({
                embed: {
                    color: 0x2ecc71,
        ...
        txMonitor.on("folderRemoved", function (folderPath) {
            txChannel.send({
                embed: {
                    color: 0x2ecc71,
        ...
        txMonitor.on("folderAdded", function (folderPath) {
            txChannel.send({
                embed: {
                    color: 0x2ecc71,
                    title: "Folder Added",
        ...
        txMonitor.on("fileChanged", function (fileDetail, changes) {
        ...
        txMonitor.on("fileAdded", function (fileDetail) {            
            txChannel.send({
                embed: {
                    color: 0x2ecc71,
        ...
        txChannel.send({
            embed: {
                color: 0x2ecc71,
                title: "Monitoring Texas PickTicket Folder!",
                fields: [{
                    name: "Monitoring mapped drive below!:",
                    value: txMonitor.root
                }
                ],
                timestamp: new Date(),
                footer: {
        ...
        console.log("Directory Monitoring of " + txMonitor.root + " has started");

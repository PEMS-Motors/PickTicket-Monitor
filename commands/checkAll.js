...
module.exports = {
    ...
    execute(message) {
        ...
        // to false
        var allMonitor = new dirwatch.DirectoryWatcher("Z:", true);

        // start the monitor and have it check for updates
        // every half second.
        allMonitor.start(60000);

        // Log to the console when a file is removed
        allMonitor.on("fileRemoved", function (filePath) {
        ...
        allMonitor.on("folderRemoved", function (folderPath) {
            message.channel.send({
                embed: {
                    color: 0x2ecc71,
        ...
        allMonitor.on("folderAdded", function (folderPath) {
            message.channel.send({
                embed: {
                    color: 0x2ecc71,
                    title: "Folder Added",
        ...
        allMonitor.on("fileChanged", function (fileDetail, changes) {
        ...
        allMonitor.on("fileAdded", function (fileDetail) {            
            message.channel.send({
                embed: {
                    color: 0x2ecc71,
        ...
        message.channel.send({
            embed: {
                color: 0x2ecc71,
                title: "Monitoring All Warehouse Location Folders!",
                fields: [{
                    name: "Monitoring mapped drive below!:",
                    value: allMonitor.root
                }
                ],
                timestamp: new Date(),
                footer: {
        ...
        console.log("Directory Monitoring of " + allMonitor.root + " has started");
